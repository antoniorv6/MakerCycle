...
const MaterialsSection: React.FC<ExtendedMaterialsSectionProps> = ({
  ...
  // Estado local para los inputs numéricos (permite que estén vacíos)
  const [inputValues, setInputValues] = useState<Record<string, { quantity?: string; cost?: string }>>({});
  ...
  const setInputValue = (itemId: string, field: 'quantity' | 'cost', value: string) => {
    setInputValues(prev => ({
      ...prev,
      [itemId]: {
        ...prev[itemId],
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  type="number"
                  placeholder="1"
                  value={getInputValue(item.id, 'quantity', item.quantity)}
                  onChange={(e) => {
                    const value = e.target.value;
                    setInputValue(item.id, 'quantity', value);
                    // Only update parent if we have a valid number
                    if (value !== '' && value !== '-' && value !== '.') {
                      const numValue = parseFloat(value);
                      if (!isNaN(numValue)) {
                        handleUpdateItem(item.id, 'quantity', numValue);
                      }
                    }
                  }}
                  onBlur={(e) => {
                    const value = e.target.value;
                    if (value === '' || value === '-' || value === '.') {
                      setInputValue(item.id, 'quantity', '0');
                      handleUpdateItem(item.id, 'quantity', 0);
                    } else {
                      const numValue = parseFloat(value);
                      if (!isNaN(numValue)) {
                        setInputValue(item.id, 'quantity', numValue.toString());
                        handleUpdateItem(item.id, 'quantity', numValue);
                      } else {
                        setInputValue(item.id, 'quantity', '0');
                        handleUpdateItem(item.id, 'quantity', 0);
              ...
              <div>
                ...
                <input
                  ...
                  value={getInputValue(item.id, 'cost', item.cost_per_unit)}
                  onChange={(e) => {
                    const value = e.target.value;
                    setInputValue(item.id, 'cost', value);
                    // Only update parent if we have a valid number
                    if (value !== '' && value !== '-' && value !== '.') {
                      const numValue = parseFloat(value);
                      if (!isNaN(numValue)) {
                        handleUpdateItem(item.id, 'cost_per_unit', numValue);
                      }
                    }
                  }}
                  onBlur={(e) => {
                    const value = e.target.value;
                    if (value === '' || value === '-' || value === '.') {
                      setInputValue(item.id, 'cost', '0');
                      handleUpdateItem(item.id, 'cost_per_unit', 0);
                    } else {
                      const numValue = parseFloat(value);
                      if (!isNaN(numValue)) {
                        setInputValue(item.id, 'cost', numValue.toString());
                        handleUpdateItem(item.id, 'cost_per_unit', numValue);
                      } else {
                        setInputValue(item.id, 'cost', '0');
                        handleUpdateItem(item.id, 'cost_per_unit', 0);
